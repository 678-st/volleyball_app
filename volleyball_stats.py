...
def build_nendo_options() -> list[str]:
    """
    H30（2018年度）から現在年度まで遡れるリストを生成。
    新年度になれば自動的に追加される。
    """
    start = 2018          # H30 = R0 の基準年
    end   = current_nendo()
    options = []
    for y in range(start, end + 1):
        ...
        if reiwa >= 1:
            ...
            # 必要なら H 表記も対応できるが、今は R1 以降のみ
            label = f"H{30 + (y - 2018)}"
        options.append(label)
    return list(reversed(options))   # 新しい年度が上に来る

NENDO_OPTIONS = build_nendo_options()
NENDO_DEFAULT = NENDO_OPTIONS[0]     # 現在年度
    ...
    with st.expander("選手を削除する"):
        if len(players) > 0:
            del_player = st.selectbox(
                "削除する選手", get_player_list(), key="del_player")
...
with st.expander("試合を作成する"):
    ...
    if st.button("試合を追加"):
        if new_match:
            ...
            st.success(
                f"「{new_nendo} / {new_type} / {new_tourn} / {new_match}」"
                f"を追加しました"
            )
            st.rerun()
        else:
...
with st.expander("試合を削除する"):
    if len(matches) > 0:
        ...
        del_label = st.selectbox(
            "削除する試合", match_labels.tolist(), key="delete_match")
        if st.button("試合を削除"):
            del_name = matches.loc[
                match_labels == del_label, "試合名"
            ].iloc[0]
            matches = matches[match_labels != del_label]
            save_matches(matches)
            st.session_state.matches = matches
...
with st.expander("試合一覧を見る"):
    if len(matches) > 0:
        show_m = (
            matches[["年度", "種別", "大会", "試合名", "日付"]]
            .sort_values(["年度", "種別", "大会"], ascending=[False, True, True])
            .reset_index(drop=True)
        )
        st.dataframe(show_m, use_container_width=True, hide_index=True)
    else:
        st.info("試合がまだ登録されていません")
...
with st.container(border=True):
    ...
    with ic1:
        input_type  = st.selectbox(
            "種別で絞る", ["すべて"] + MATCH_TYPES, key="input_type")
    with ic2:
        input_tourn = st.selectbox(
            "大会で絞る", ["すべて"] + TOURNAMENTS,  key="input_tourn")
    with ic3:
        input_nendo = st.selectbox(
            "年度で絞る", ["すべて"] + NENDO_OPTIONS, key="input_nendo")

    mf = matches.copy()
    if input_type  != "すべて": mf = mf[mf["種別"] == input_type]
    if input_tourn != "すべて": mf = mf[mf["大会"] == input_tourn]
    if input_nendo != "すべて": mf = mf[mf["年度"] == input_nendo]
    ...
    sel_label = st.selectbox("試合", mf_labels.tolist(), key="input_match")
    sel_row   = mf.loc[mf_labels == sel_label].iloc[0]
...
with fa1:
    f_type  = st.selectbox(
        "① 種別", ["すべて"] + MATCH_TYPES, key="f_type")
with fa2:
    f_tourn = st.selectbox(
        "② 大会", ["すべて"] + TOURNAMENTS,  key="f_tourn")
with fa3:
    f_nendo = st.selectbox(
        "③ 年度", ["すべて"] + NENDO_OPTIONS, key="f_nendo")
    ...
    if len(filtered_df) == 0:
        ...
        if filtered_df["試合名"].nunique() > 1:
            ...
            for mname in filtered_df["試合名"].unique():
                ...
                a     = int((mdf["評価"] == "A").sum())
                d     = int((mdf["評価"] == "D").sum())
                minfo = matches[matches["試合名"] == mname]
                if len(minfo) == 0:
                    continue
                row = minfo.iloc[0]
                match_rows.append({
